    ...
    for (let i = 0; i < arr.length - 1; i++) {
        for (let j = i + 1; j < arr.length; j++) {
            for (let k = j + 1; k < arr.length; k++) {
                if (SUM_VALUE === arr[i] + arr[j] + arr[k] && !indexes.includes(i) && !indexes.includes(j) && !indexes.includes(k)) {
...
const twoSumResult = twoSum(input);
console.log(`twoSum Solution ==> `, twoSumResult);

const threeSumResult = threeSum(input);
console.log(`threeSum Solution ==> `, threeSumResult[0] * threeSumResult[1] * threeSumResult[2]);